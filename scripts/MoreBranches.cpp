...
namespace minitrees
{
	...
	void MakeMiniTree(string MiniTreeName)
	{
		...
		for(It=minitrees::datasets.begin(); It!=minitrees::datasets.end(); It++)
		{
			...
			if( access(dspath.c_str(), R_OK|X_OK)!=0 ) continue;
	...
	void ProcessEvent(Event *ev)
	{
		...
		data.ResetS1peaks(nS1);
		data.ResetS2peaks(nS2);
		
		if(nPeaks<nS1s+nS2s){//Something is clearly wrong!
			return;
		}
		
		//Extract data from S1 signals:
		data.S1s_vec->resize();
	...
	void ExtractS1info(Event* peak, int peakindex)
	{
		...
		int ncoinsbot = ncoins - ncoin
		
		double vector<PmtContrib> pmtscontrib;//This must be sorted from the largest to the smallest
